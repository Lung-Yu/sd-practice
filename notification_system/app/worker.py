...
def _wait_for_redis(r: redis.Redis, retries: int = 60, delay: float = 2.0) -> None:
...
def run() -> None:
    ...
    # Expose delivery-side Prometheus metrics on a dedicated port.
    # The API workers expose API-side metrics on :8000/metrics; this gives
    # Prometheus a separate scrape target for delivery counters/histograms.
